import { test, expect, seedState, readSessions, isoOffset } from './helpers'
...
test('a valid backup restores after confirmation', async ({ page }) => {
  await seedState(page, { settings: { currentPhaseId: 'beginner', phaseStartDate: isoOffset(0) } })
  await page.goto('/settings')
  ...
  await importFile(page, {
    ...
    settings: [{ id: 'app', dbIncrement: 2, currentPhaseId: 'beginner', phaseStartDate: isoOffset(0), onboarded: true }],
    maxes: [{ liftId: 'op_bench', testWeight: 20, testReps: 5, bumpKg: 0 }],
    sessions: [{ date: isoOffset(-2), phaseId: 'beginner', week: 1, day: 0, type: 'lift', title: 'Restored Lift', exercises: [], done: true, createdAt: 1 }],
  })
