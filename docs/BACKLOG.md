...
| ID | Item | Evidence |
|---|---|---|
| A5 | **"Load demo history" destroys real data in one tap** — no confirm, no DEV gate, no undo, directly below Export, and it sets `lastBackupAt` so the backup nudge goes quiet afterwards. | code-01 F1 |
...
| A15 | **Dead "Resume" button at the end of a plan** — `realign` writes `phaseStartDate`, which `resolveInPlan` never reads, so Today bricks with "It's been 0 days". No reassess/next-cycle prompt either (p.140, p.147). | book-04 F9, code-03 F3 |
